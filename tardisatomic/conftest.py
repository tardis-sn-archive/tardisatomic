...
def pytest_addoption(parser):
    ...
    parser.addoption("--atomic-dataset", dest='atomic-dataset',
                     default=None, help="filename for atomic dataset")
...
import pandas as pd
@pytest.fixture
def atomic_levels(atomic_data_fname):
    return pd.DataFrame(h5py.File(atomic_data_fname, 'r')
                        ['levels_data'].__array__())
@pytest.fixture
def atomic_lines(atomic_data_fname):
    return pd.DataFrame(h5py.File(atomic_data_fname, 'r')
                        ['lines_data'].__array__())
@pytest.fixture
def atomic_ionization_data(atomic_data_fname):
    return pd.DataFrame(h5py.File(atomic_data_fname, 'r')
                        ['ionization_data'].__array__())