...
class IngestGFAll(BaseIngest):
    ...
    def ingest(self, fname):
        ...
        value_type_wl = TransitionValueType(name='wavelength', unit_id=units_angstrom, dtype_id=dtype_float)
        value_type_loggf = TransitionValueType(name='loggf', unit_id=units_none, dtype_id=dtype_float)
        ...
        levels['alchemy_level'] = pd.Series(np.random.randn(len(levels)), index=levels.index)
        for i, row in levels.iterrows():
            clevel = Level(ion_id=row['level_id'], level_number=row['level_number'], g=row['g'], energy=row['energy'],
                           label=row['label'], theoretical=row['theoretical'], data_source_id=data_source_ku)
        ...
        for i, row in lines.iterrows():
            ...
            clineT = Transition(transition_type_id=transition_type_line, source_level_id=upperlevel,
                                target_level_id=lowerlevel, data_source_id=data_source_ku)
            clinegf = TransitionValue(transition_id=clineT, transition_value_type_id=value_type_loggf,
                                      value=row['loggf'], data_source_id=data_source_ku)
            vacum_wl = self._convert_air_to_vacuum(row['wavelength'])
            clinewl = TransitionValue(transition_id=clineT, transition_value_type_id=value_type_wl, value=vacum_wl,
                                      data_source_id=data_source_ku)
